...
# Навигация
# ==========================
async def on_resume_selected(cq: CallbackQuery, _w: Any, dialog_manager: DialogManager, item_id: str):
    """
    Выбор элемента из Select: сохраняем resume_id и переходим в окно управления.
    """
    dialog_manager.dialog_data["resume_id"] = item_id
    await dialog_manager.switch_to(ResumesSG.manage)

async def to_pos_input(_cq: CallbackQuery, _btn: Button, dialog_manager: DialogManager):
    """Переход к вводу позитивных ключевых слов."""
    await dialog_manager.switch_to(ResumesSG.pos_input)

async def to_neg_input(_cq: CallbackQuery, _btn: Button, dialog_manager: DialogManager):
    """Переход к вводу негативных слов."""
    await dialog_manager.switch_to(ResumesSG.neg_input)

async def toggle_status(cq: CallbackQuery, _btn: Button, dialog_manager: DialogManager):
    """
...
async def ask_delete(_cq: CallbackQuery, _btn: Button, dialog_manager: DialogManager):
    """Открыть окно подтверждения удаления."""
    await dialog_manager.switch_to(ResumesSG.confirm_delete)

async def do_delete(_cq: CallbackQuery, _btn: Button, dialog_manager: DialogManager):
    """Удалить резюме и вернуться к списку."""
    rid = dialog_manager.dialog_data.get("resume_id")
    if rid:
        await Resume.filter(id=rid).delete()
    await dialog_manager.switch_to(ResumesSG.list)
...
async def on_pos_words(msg: Message, _inp: MessageInput, dialog_manager: DialogManager):
...
async def on_neg_words(msg: Message, _inp: MessageInput, dialog_manager: DialogManager):
